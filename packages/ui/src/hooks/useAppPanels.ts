...
} from "@/lib/sidebarVisibilityPreference.js";
...
export function useAppPanels(options: {
  ...
  const [isSidebarVisible, setIsSidebarVisible] = useState(readSidebarVisiblePreference);
  // Bugfix：刷新后侧栏总是回到展开态，用户收起的选择丢失。开合状态现在持久化到
  // localStorage（读写失败静默忽略），刷新/重载后保持上次的展开或收起。
  useEffect(() => {
    saveSidebarVisiblePreference(isSidebarVisible);
  }, [isSidebarVisible]);
